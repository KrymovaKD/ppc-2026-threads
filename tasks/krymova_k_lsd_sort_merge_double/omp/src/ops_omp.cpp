...
void KrymovaKLsdSortMergeDoubleOMP::LSDSortDoubleParallel(double *arr, int size) {
  if (size <= 1) {
    return;
  }
...
#pragma omp parallel for
  ...
  for (int pass = 0; pass < k_passes; ++pass) {
    ...
    std::fill(global_count.begin(), global_count.end(), 0U);
...
#pragma omp critical
    ...
    std::copy(global_count.begin(), global_count.end(), pos.begin());
...
#pragma omp parallel for
  for (int i = 0; i < size; i += portion) {
    ...
    LSDSortDoubleParallel(arr + i, current_size);
  }
}
